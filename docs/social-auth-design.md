...
- BcAuthPasskey と同時に導入しても UI やログイン完了処理が競合しにくい構成にする
...
**BcAuthSocial（基盤）＋ 外部アドオン登録方式** を採用します。

- **BcAuthSocial** に Google と X を同梱し、単体で動作する
...
各アドオンは `BcAuthSocial` に依存するが、`BcAuthSocial` 側はアドオンを知らない（依存逆転）。
...
そのため、BcAuthSocial は OIDC 専用設計ではなく、OAuth 2.0 / OIDC の両方を扱える ProviderAdapter 前提で進める必要があります。

## BcGoogleLogin 5 系移行方針

BcGoogleLogin をそのまま 5 系へ移植するのではなく、次のどちらかで整理する想定です。

1. BcAuthSocial の Google Provider として統合する
...
provider ごとの設定値は `BcAuthSocial.providers.{provider}` 配下で扱います。
初期実装では [plugins/BcAuthSocial/config/setting.php](plugins/BcAuthSocial/config/setting.php) から env を読み込みます。
...
- GET /baser/admin/bc-auth-social/auth/{provider}/login
- GET /baser/admin/bc-auth-social/auth/{provider}/callback
- POST /baser/admin/bc-auth-social/auth/{provider}/disconnect
- GET /baser/admin/bc-auth-social/provider_links/index

### Front

- GET /bc-auth-social/auth/{provider}/login
- GET /bc-auth-social/auth/{provider}/callback
...
BcAuthSocial の `config/bootstrap.php` で Google と X を登録します：

```php
// plugins/BcAuthSocial/config/bootstrap.php
...
BcAuthSocial 側は BcLineAuth を知らず、BcLineAuth 側が BcAuthSocial に依存する依存逆転の構成になります。
...
- `BcAuthSocial.google.Admin`
- `BcAuthSocial.x.Admin`

これにより、複数 provider を並行利用した場合でも状態が衝突しにくくなります。

## BcAuthCommon との境界

BcAuthSocial 自体は OAuth / OIDC に集中し、次の責務は共通側へ寄せる想定です。
...
- BcAuthPasskey と同時導入時も UI が破綻しないことを優先する
...
- BcAuthPasskey と BcAuthSocial の共存に向く
- 将来的なプロバイダ追加がしやすい
...
### BcAuthSocial（基盤プラグイン）

```
plugins/BcAuthSocial/
├── src/
│   ├── BcAuthSocialPlugin.php
│   ├── Controller/
│   │   ├── Admin/
...
BcAuthSocial を require し、bootstrap.php で登録するだけの最小構成です。
...
├── composer.json                           # "require": { "baserproject/bc-auth-social": "^1.0" }
└── README.md
```
...
BcAuthSocial は **基盤プラグイン＋外部アドオン登録方式** を採用します。
...
BcAuthPasskey と同時利用される前提で、初期段階では template override を用いつつ、将来的にはログイン画面の共通入口とログイン完了処理の共通化へ寄せる設計を採用します。
